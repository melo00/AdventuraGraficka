package com.github.melo00.Adventura.main;

import com.github.melo00.Adventura.logika.Hra;
import java.util.Observable;
import java.util.Observer;

import com.github.melo00.Adventura.logika.Prostor;
import com.github.melo00.Adventura.logika.Vec;
import java.net.URL;
...
public class HomeController implements Observer, Initializable {
	...
	@Override
        ...
        @FXML public void klikBatoh() 
        {
                    ...
                    seznam = hra.getHerniPlan().getBatoh().vratObsahBatohu();
                    int index = seznamVeciBatoh.getSelectionModel().getSelectedIndex();
                    
                    String nazev = "";
                    int pomocna = 0;
                    for (String jmeno : seznam.keySet()) 
                    {
                       if(pomocna == index)
                       {
                           nazev = jmeno;
            ...
            if(!hra.konecHry())
            {
            vstupniText.setText("vyhod " + nazev);
            odesliPrikaz();
            }
        }
         ...
         @FXML public void Napoveda() 
        {
            ...
            webView.getEngine().load(com.github.melo00.Adventura.main.GuiAdventura.class.getResource("/zdroje/napoveda.html").toExternalForm());
	...
	public void update(Observable arg0, Object arg1) 
		...
		String sVychody = hra.getHerniPlan().getAktualniProstor().seznamVychodu();
                ...
                Map<String, Vec>sBatoh = hra.getHerniPlan().getBatoh().vratObsahBatohu();
                for (String nazev : sBatoh.keySet()) 
                {
                    Vec pomocna = sBatoh.get(nazev);
                    ImageView obrazek = new ImageView(new Image(com.github.melo00.Adventura.main.GuiAdventura.class.getResourceAsStream("/zdroje/"+pomocna.getUrl()), 100, 100, false, false));
                    veciBatoh.add(obrazek);
                }
                
                Map<String, Vec> sVeci = hra.getHerniPlan().getAktualniProstor().getVeci();
                for (String nazev : sVeci.keySet()) 
                {
                    Vec pomocna = sVeci.get(nazev);
                    ImageView obrazek = new ImageView(new Image(com.github.melo00.Adventura.main.GuiAdventura.class.getResourceAsStream("/zdroje/"+pomocna.getUrl()), 100, 100, false, false));
